# Sprint 27.1-J – Repairs & Re-Audits

## Codex-Ausführungsanweisung

```text
Execute Sprint 27.1-J only.
...
- every audit file referenced by RQ-13-01
- every audit file referenced by RQ-17-01
...
If RQ-17-01 is a prerequisite for RQ-13-01, reverse the order.
...
Execute the canonical repair defined for RQ-13-01.
...
Execute the canonical repair defined for RQ-17-01.
...
If this repair shares implementation with RQ-13-01:
...
Inspect:
- all previously completed Repair IDs in CURRENT REPAIR_QUEUE.md
- especially repairs touching the same files/components
- prior RQ-13 repairs already marked implemented/re-audited
- prior Sprint 17 / 17.x related repairs if RQ-17-01 affects shared frontend,
  cache, layout, focus or navigation behavior
...
If 27.1-J changes behavior present in that Public Test Release:
...
- remove only RQ-13-01 / RQ-17-01 where now resolved
...
Do NOT silently expand 27.1-J scope.

ACCEPTANCE GATE – 27.1-J

Before commit verify ALL of the following:
- RQ-13-01 acceptance criteria satisfied as far as automatable
- RQ-17-01 acceptance criteria satisfied as far as automatable
...
- git diff contains only intended 27.1-J changes

If any acceptance item fails:
- do NOT present 27.1-J as complete
...
COMMIT & PUSH – BATCH 27.1-J

If the 27.1-J acceptance gate passes:
...
Stage ONLY:
- product changes required for RQ-13-01
- product changes required for RQ-17-01
...
git commit -m "fix(audit): repair RQ-13-01 and RQ-17-01"
...
2. Confirm RQ-13-01 has accurate status.
3. Confirm RQ-17-01 has accurate status.
...
1. starting commit
2. branch
3. canonical definition summary for RQ-13-01
4. canonical definition summary for RQ-17-01
...
38. 27.1-J acceptance gate result
39. commit hash
40. push result
...
Do NOT start Sprint 27.1-K automatically.
```
...
git diff --check
git diff
git add <nur Dateien für RQ-13-01, RQ-17-01 und zugehörige Tests/Audit-Dokumentation>
git commit -m "fix(audit): repair RQ-13-01 and RQ-17-01"
git push
```
