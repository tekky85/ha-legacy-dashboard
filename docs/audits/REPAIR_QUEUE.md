...
- Code repairs completed: 1 (`RQ-16-01`)
- Open code repairs: 24
- P0: 0
- P1 open: 6
- P1 code-closed/manual-pending: 1
- P2: 18
- P3: 0
- RC Blocker `YES` open: 5
- RC Blocker code-closed/manual-pending: 1
- RC Blocker `CONDITIONAL`: 5
- Security-sensitive repairs open: 10; code-closed/manual-pending: 1
- Repairs blocking manual tests: 13
- Repairs with dependencies: 9
- Status: **SPRINT 27.1-A COMPLETE – 27.1-B NEXT**
...
| RQ-04-01 | 17.2, 17.3, 18, 19, 20, 21, 21.1, 21.2, 21.3, 21.4, 21.5, 22, 23, 24, 25, 25.1, 25.2, 25.3, 25.5, 25.6, 25.7, 26, 26.1, 26.2 | Gemeinsam genutzte Legacy-Assets besitzen zwischen Dashboard, Systemseiten und Admin unterschiedliche Cache-Buster. | PARTIAL | P1 | Ursprungsbefund Part 04: `src/public/index.html` lädt `theme.js`, `style.css`, `compat.js` und `system-navigation.js` mit `v=51`, `src/public/system.html` dieselben Assets mit `v=44`. Zusätzliche Part-07- bis Part-19-Evidenz: `src/admin/index.html` lädt geteilte Public-Assets mit `v=50`, das Dashboard mit `v=51`; Summary, Errors und spätere Systemfunktionen werden über die Systemseite mit `v=44` gerendert. Sprint 25.3 rendert Background, Titel, Vollhöhe, Footer und Focus im Dashboard intern konsistent mit v51, aber Admin-Preview, Theme und routeübergreifende Navigation verwenden gemeinsam genutzte Assets mit abweichenden immutable URLs. Für Sprint 25.7 ist dies besonders kritisch: Ein iOS-9-HomeScreen-Web-App-Kontext kann nach einem Update alte Theme-, Same-Window-, Return-, Layout- oder Control-Dateien routeabhängig behalten. Part 19 bestätigt dieselbe Wirkung auf Section-Gruppierung, Room-Renderer/-Background/-Collapse sowie capabilityabhängige Light-/Climate-/Target-Controls: ein neuer Backendstand kann mit altem Shared-CSS/Controller dargestellt werden. Das App-Dockerfile und das Standalone-Bundle verteilen denselben `src`-Baum; weder Release-Gate noch die Node-Tests erkennen diesen Cachezustand. Public-Assets werden `immutable` ausgeliefert; es gibt keinen zentralen Versionsmechanismus. | Alle gemeinsam genutzten Public-Assets auf einen konsistent erhöhten Wert setzen; automatisierten Gleichheitstest über Dashboard/System/Admin ergänzen; unzutreffende Statusaussagen nach Re-Audit aktualisieren. | Muss vor RC und vor der verbindlichen Kiosk-Abnahme behoben werden, weil Legacy Safari sonst nach einem Rollout routeabhängig alte Theme-, Same-Window-, Return-, Health-, Background-/Preview-, Section-, Room-, Presentation- oder Control-Logik dauerhaft aus dem Immutable-Cache laden kann. | offen |
...
| RQ-12-01 | 22 | `ruleSource` bezeichnet bei gemischten Domain-/Risk-Regeln nicht zuverlässig die tatsächlich wirksame Feldquelle. | BROKEN | P2 | `resolveRule()` setzt nach einer wirksamen Domain-Regel `source` bereits beim Vorhandensein der Risk-Class-Regel neu. Kontrollierte Probe mit `domains.sensor.expectedOffline=true`: effektiver Wert ist `true`, gemeldet wird aber `ruleSource="risk_class"`. Der bestehende Prioritätstest erwartet diese falsche Zuordnung sogar. | Herkunft pro wirksamem Feld sauber bestimmen oder das öffentliche Modell eindeutig als Klassifikations-/letzte Merge-Schicht umbenennen; gezielte Domain/Risk/Device/Entity-Mischtests ergänzen, ohne die korrekte Wertpriorität zu verändern. | Diagnose-/Erklärbarkeit ist falsch, Regelwirkung und Security bleiben korrekt. Vor RC als Korrektheitsbefund beheben. | offen |
| RQ-12-02 | 23 | Rein dynamische/nicht auflösbare Automationreferenzen erhalten keinen produktiven Impact mit Confidence `unknown`. | MISSING | P2 | `CONFIDENCE_ORDER` und `impactConfidence()` kennen `unknown`, `Indexes.create()` sammelt `dynamicAutomationEntityIds`; `AutomationImpact.forIssue()` liest diese Liste jedoch nie. Direktprobe: `dynamicReferences=true`, `dynamicCount=1`, Impact für ein Issue `[]`. Der bestehende Test bestätigt ausdrücklich, dass die dynamische Automation nicht erscheint. | Kausalitätsfreie Darstellung für nicht zuordenbare dynamische Referenzen entwerfen: entweder separater Unknown-/Unvollständigkeitskontext je betroffener Ansicht oder klarer globaler Abschnitt; niemals jede dynamische Automation als Ursache einer Entity behaupten. Test für tatsächlich erreichbaren `unknown`-Pfad ergänzen. | Diagnosevollständigkeit unzureichend, aber konservativ und ohne falsche Write-/Securitywirkung. Vor RC fachlich entscheiden und implementieren. | offen |
| RQ-12-03 | 23 | Der Reference-Cache liefert für Automation Impact bis zum TTL veralteten Automation-State/Disabled-Kontext und `lastTriggered`. | BROKEN | P2 | Bei Config-Cache-Hit merged `getMetadata()` zwar das frische Inventory, gibt aber unverändert `configCache.indexes` zurück. `Impact.forIssue()` liest Metadaten aus dessen altem `inventoryByEntityId`. Direktprobe innerhalb 60 s: öffentliches Inventory `off`, Impactindex weiterhin `on` und alte Triggerzeit. | Bei jedem Snapshot den Index mit frischem Inventory und gecachten Referenzen neu bauen oder mindestens `inventoryByEntityId` atomar aktualisieren; Referenz-TTL beibehalten; Regression `on↔off`, Name und `lastTriggered` innerhalb TTL ergänzen. | Sichtbarer Diagnosekontext kann bis 60 s falsch sein; keine HA-Write- oder Credentialauswirkung. Vor RC beheben. | offen |
...
| RQ-15-01 | 25.1, 25.2 | Die nummerierten 74-/51-Punkte-Release-Gate-Matrizen sind fachlich breit, aber nicht vollständig als direkte Einzelanforderungen rückverfolgbar. | PARTIAL | P2 | `test/sprint-17-2.test.js` prüft die globale Theme-Persistenz in fünf breiten Fällen, `test/system-frontend.test.js` die exakten Filter in einem umfangreichen Tabellenlauf und `test/sprint-25-2.test.js` die Navigation in fünf breiten Fällen. Part-15-Fokuslauf 65/65 und Gesamtsuite 329/329 sind grün. Dennoch sind reale HomeScreen-/Safari-Punkte nur über MT-13/34/40–42 abgedeckt, und kein Test erkennt die routeabhängige Assetversionsabweichung aus `RQ-04-01`. | Alle 74 bzw. 51 Anforderungen einem direkten Test, einer dokumentiert äquivalenten Regression oder einem konkreten Manual-Test zuordnen; konsistente Assetversionen ausdrücklich regressieren; HomeScreen-Resultate commit-/versionbezogen dokumentieren. Nur localhost-Mocks/Fake-Credentials verwenden. | Testhärtung und belastbares Release Gate vor Stable; kein zusätzlich bestätigter fachlicher Laufzeitdefekt. | offen |
...
|---|---|---|---|---|---|---|---|---|
| RQ-04-01 | Public HTML, gemeinsame Wall-CSS/JS, Static-Cache-Header, Admin-Preview | YES | NO | – | RQ-08-02, RQ-15-01; verbindliche Legacy-Abnahme | MT-01/02/05/06/09/11–16/18–25/28/32–34/39–42/45/48/54/58/63–69/71/72 | Sprint 17.2/17.3, 18–26.2 soweit im Finding genannt | 27.1-B |
...
| RQ-08-02 | README-Galerie, echte Screenshots, Dateiformate/Datenschutz | CONDITIONAL – vor öffentlicher RC-Dokumentation | YES | RQ-04-01, RQ-09-01, RQ-12-01/02/03, RQ-18-01 | finaler Dokumentationsstand | MT-29 | D1 sowie Screenshot-Findings in Sprint 21–23, 25.1, 26/26.1 | 27.1-I |
...
| RQ-09-01 | Backend-HA-WebSocket-Lifecycle/Reconnect | NO | YES | – | RQ-09-02, RQ-12-04; reale Registry-/Automation-/HAOS-Recovery | MT-30/35/36/46/50 | Sprint 21, 21.3, 23, 24, 25.4 | 27.1-C |
...
| RQ-12-01 | Rule Engine `ruleSource`/Erklärbarkeit | NO | NO | – | RQ-12-04; reale Regelabnahme | MT-43–45 | Sprint 22 | 27.1-C |
| RQ-12-02 | Automation Impact, Dynamic-/Unknown-Kontext, Sanitizer/UI | NO | YES | – | RQ-12-04; reale Automation-Abnahme | MT-46–49 | Sprint 23 | 27.1-C |
| RQ-12-03 | Automation-Reference-Cache und frisches Inventory-Merge | NO | YES | – | RQ-12-04; reale Automation-Abnahme | MT-46–49 | Sprint 23 | 27.1-C |
...
| RQ-13-01 | Versionen, Tag/Commit, GHCR, App-Image, Release Notes | YES | YES | RQ-04-01, RQ-13-02, RQ-14-01/02/04/05, RQ-18-01 | RQ-17-01; aktuelle HAOS-/Release-Abnahme | MT-50–62/64–66/70 | Sprint 24, 25, 25.4 | 27.1-J |
...
| RQ-15-01 | Theme-/Exact-Filter-/HomeScreen-Testmatrix | NO | NO | RQ-04-01 | belastbarer Sprint-25.1/25.2-Re-Audit | MT-13/34/40–42 | Sprint 25.1/25.2 | 27.1-F |
...
- **P1 offen: 6.** RQ-04-01 gefährdet den tatsächlichen Legacy-Clientstand;
  RQ-13-01 verteilt alten Code; RQ-14-01/-02 betreffen installierbare und
  ...
  lassen. RQ-16-01 ist code-seitig geschlossen, bleibt aber bis zu den
  zugeordneten Realtests als `MANUAL PENDING` sichtbar.
...
- Offene Code-Repair-Einträge: 24; zusätzlich ein code-seitig geschlossener,
  manuell noch nicht abgenommener Eintrag (`RQ-16-01`).
- Offene Prioritäten: P0 0, P1 6, P2 18, P3 0.
