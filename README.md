# HA Legacy Dashboard

A lightweight external Home Assistant dashboard for legacy browsers and wall displays.

It can run either as a standalone Node.js/LXC gateway or as a local
Home Assistant App. Both modes keep Home Assistant credentials exclusively in
the backend and provide direct LAN access for legacy iPads.

Choose an installation:

- Home Assistant OS: [add the Custom App Repository](https://my.home-assistant.io/redirect/supervisor_add_addon_repository/?repository_url=https%3A%2F%2Fgithub.com%2Ftekky85%2Fha-legacy-dashboard)
- LXC, VM, or Linux server: use the versioned standalone release bundle

## Documentation / Dokumentation

- 🇩🇪 [Deutsch](./README.de.md)
- 🇬🇧 [English](./README.en.md)
- [Home Assistant App documentation](./ha_legacy_dashboard/DOCS.md)
- [Standalone and App deployment](./docs/DEPLOYMENT.md)
- [Release and distribution process](./docs/RELEASING.md)
- [Public Test Release 1.0.0-rc.7](./docs/PUBLIC_TEST_RELEASE.md)
- [GitHub Releases](https://github.com/tekky85/ha-legacy-dashboard/releases)

## Screenshots

![HA Legacy Dashboard – Light Mode](docs/screenshots/dashboards/main-light.png)

The repository uses real screenshots of the running application. See the language-specific README files for the complete gallery and its maintenance rules. The [reproducible capture manifest](docs/screenshots/README.md) records routes, viewports, provenance, and privacy checks.
